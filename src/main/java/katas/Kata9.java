        ...
        return movieLists.stream()
                ...
                .flatMap(movies -> movies.stream()
                        .map(movie -> ImmutableMap.of("id", movie.getId(),
                                ...
                                "time" , movie.getInterestingMoments().stream().filter(interestingMoment ->
                                        interestingMoment.getType().equals("Middle")).collect(Collectors.toList()).get(0).getTime() ,
                                "boxart", movie.getBoxarts().stream().reduce((boxArt, boxArt2) ->
                                        boxArt.getHeight()*boxArt.getWidth() > boxArt2.getHeight()*boxArt2.getWidth() ?
                                                boxArt : boxArt2).get().getUrl())))
